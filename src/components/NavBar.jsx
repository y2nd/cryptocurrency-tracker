        ...
        {isOpen && 
            <div className="md:hidden fixed left-0 top-20 flex flex-col items-center justify-between w-full h-[90%] bg-primary ease-in duration-300 z-10">
                <ul className="w-full p-4">
                    ...
                    { user?.email  && (<li className="border-b py-6">
                        <Link onClick={() => setIsOpen(!isOpen)} to={"/account"}>Account</Link>
                    </li>)}
                    <li className="border-b py-6">
                        <ThemeToggle />
                    </li>
                ...
                <div className="flex flex-col w-full p-4">
                    {user?.email ? (
                        <div className="">
                            <Link to={"/account"} onClick={() => setIsOpen(!isOpen)}>
                                <button className=" bg-yellow-600 t
                                w-full my-2 p-3 text-primary border border-secondary rounded-2xl shadow-xl">Account</button>
                            </Link>
                            <Link to={''} onClick={
                                () => {
                                    handleSignOut(); 
